...
| -------------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------- |
| AKShare              | https://github.com/akfamily/akshare @ PyPI `1.18.91`                                           | MIT                                                                         | Eastmoney primary and Sina fallback SDK calls for unadjusted A-share daily bars, plus Sina historical trading dates for development/evaluation | Use the locked SDK behind Zhaoniu provider ports. Keep retries, fallback eligibility, source lineage, calendar session semantics, unit conversion, canonical validation and persistence product-owned. | None        |
...
LiteLLM is constrained to `>=1.95,<2` and resolved to `1.97.0` in `uv.lock`. An upgrade requires
reviewing its root and enterprise license boundaries, provider parameter compatibility, structured
output behavior and exception taxonomy, then rerunning all gateway contract and fallback tests.

AKShare is constrained to `>=1.17,<2` and resolved to `1.18.91` in `uv.lock`. An upgrade requires
reviewing the exact Eastmoney and Sina function signatures and schemas, including
`tool_trade_date_hist_sina`, volume units, exception behavior and upstream identity, then rerunning
primary, fallback, calendar, normalization and quality tests.
