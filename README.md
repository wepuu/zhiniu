...
当前仓库只实现 **Phase 0 — Foundation**：可持续迭代的工程结构、边界、Mock API、响应式 UI 骨架、数据库模型与质量门禁。没有接入真实行情、AI 研究或支付。
...
packages/
  api-client/          future OpenAPI-generated TypeScript client
...
Format code with `pnpm format` and `uv run ruff format .`. Generate an OpenAPI artifact for the future client with:
